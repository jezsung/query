...
/// - A concrete [StaleDurationDuration] (time-based Duration)
...
/// ├── StaleDurationDuration (extends Duration)
/// ├── StaleDurationInfinity
/// └── StaleDurationStatic
/// ```
///
/// For dynamic stale durations based on query state, use the separate
...
sealed class StaleDuration {
  ...
  const factory StaleDuration({
    ...
    int milliseconds,
    int microseconds,
  }) = StaleDurationDuration._;

  /// Zero-duration stale time (data is immediately stale).
  ///
  /// Equivalent to `const StaleDuration()` but more explicit.
  ///
  /// Aligned with TanStack Query's `staleTime: 0`.
  static const StaleDuration zero = StaleDurationDuration._();
...
class StaleDurationDuration extends Duration implements StaleDuration {
