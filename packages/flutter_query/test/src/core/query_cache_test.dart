...
void main() {
  ...
  group('findAll with QueryTypeFilter', () {
    late QueryClient client;
    late QueryCache cache;
    ...
    test('SHOULD return all queries WHEN type is all', () {
      // Create inactive query (no observer)
      cache.build(QueryOptions<String, Object>(
        const ['inactive'],
        (_) async => 'data',
      ));
      ...
      final queries = cache.findAll(type: QueryTypeFilter.all);
    ...
    test('SHOULD return only active queries WHEN type is active', () {
      // Create inactive query (no observer)
      cache.build(QueryOptions<String, Object>(
        const ['inactive'],
        (_) async => 'data',
    ...
    test('SHOULD return only inactive queries WHEN type is inactive', () {
      // Create inactive query (no observer)
      cache.build(QueryOptions<String, Object>(
        const ['inactive'],
        (_) async => 'data',
    ...
    test('SHOULD consider disabled observer as inactive', () {
      ...
      final activeQueries = cache.findAll(type: QueryTypeFilter.active);
    ...
    test('SHOULD combine type filter with queryKey filter', () {
      // Create inactive users query
      cache.build(QueryOptions<String, Object>(
        const ['users', 'inactive'],
        (_) async => 'data',
      ));
